...
template<>
struct std::hash<MrCorpt::Eval::NNUE::EvalFile> {
    std::size_t operator()(const MrCorpt::Eval::NNUE::EvalFile& evalFile) const noexcept {
        std::size_t h = 0;
        MrCorpt::hash_combine(h, evalFile.defaultName);
        MrCorpt::hash_combine(h, evalFile.current);
        MrCorpt::hash_combine(h, evalFile.netDescription);
        return h;
    }
};
