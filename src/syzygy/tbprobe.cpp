...
using namespace MrCorpt::Tablebases;

int MrCorpt::Tablebases::MaxCardinality;
...
uint8_t* set_sizes(PairsData* d, uint8_t* data) {
    ...
    for (std::size_t i = 0; i < d->symlen.size(); ++i)
        if (!visited[i])
            d->symlen[i] = set_symlen(d, static_cast<Sym>(i), visited);
